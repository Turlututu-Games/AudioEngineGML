...
- Public functions: `PascalCase` (e.g., `AudioEngineGamePlay`, `AudioEngineDefineMusic`)
- Private/internal functions: `__PascalCase` with double underscore prefix (e.g., `__AEBus`, `__AudioEngineSystem`)
- Local variables: `_camelCase` with single underscore prefix (e.g., `var _busName`, `var _newMusicSingle`)
- Struct/constructor member fields: `camelCase` without prefix (e.g., `cleanOnRoomEnd`, `isStream`)
- Macros/constants: `__UPPER_SNAKE_CASE` with double underscore prefix (e.g., `__AUDIOENGINE_PREFIX_MUSIC`)
- Enums: `UPPER_SNAKE_CASE` with `AE_` prefix (e.g., `AE_MUSIC`, `AE_CATEGORIES`)
- Prefix resource names based on their type, e.g. `obj_player`, `spr_player`, etc.
- Internal objects use `__o` prefix (e.g., `__oAudioEngineManager`)
